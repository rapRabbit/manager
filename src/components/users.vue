      ...
      <el-col :span="8">
        <div class="grid-content bg-purple">
          <el-input placeholder="请输入内容" v-model="pageData.query">
            <template slot="append" type="primary">
        ...
        <el-table :data="userList" style="width: 100%" border>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              ></el-button>
              <!-- 角色分配按钮 -->
              <el-button type="warning" @click="showRoleDialog(scope.row)" plain size="mini" icon="el-icon-check"></el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-col>
      ...
      <el-form label-position="left" label-width="100px" ref="editForm">
        ...
        <el-form-item label="请选择角色">
          <el-select v-model="selectUser.role_name" placeholder="请选择">
            <el-option v-for="item in roles" :key="item.id" :label="item.roleName"
...
export default {
    ...
    return {
      ...
      rules: {
        username: [
          ...
          { min: 3, max: 32, message: "长度在3到32 个字符", trigger: "blur" }
        ],
        password: [
          { required: true, message: "请输入密码", trigger: "blur" },
          { min: 6, max: 32, message: "长度在3 到 32 个字符", trigger: "blur" }
        ]
      },
      //编辑框是否可见
      editVisible: false,
  ...
  methods: {
    ...
    delOne(data) {
      ...
      let id = data.id;
      //提示用户
      this.$confirm("此次操作将永久删除这个用户,是否继续?", "提示", {
