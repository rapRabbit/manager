              ...
              <el-row v-for="item in props.row.children" :key="item.id">
                ...
                <el-col :span="20">
                  ...
                  <el-row v-for="level2  in item.children" :key="level2.id">
                    <el-col :span="4">
                      <el-tag @close="delCurrentTag(props.row,level2)" closable type="success">{{level2.authName}}</el-tag>
                      <!-- 小箭头 -->
                      <i class="el-icon-arrow-right"></i>
                    </el-col>
                    <!-- 三级菜单 跟二级是平齐的 -->
...
export default {
  ...
  methods: {
    ...
    delOne(data) {
      ...
      let id = data.id;
      //提示用户
      this.$confirm("此操作将永久删除这个角色,是否继续?", "提示", {
    ...
    async showRolesDialog(data) {
      ...
      let findChild = father => {
        ...
        if (father.children) {
          ...
          //没有后代
          console.log("没有后代啦");
          //因为tree这个组件的问题 如果父级id 设置被选中 会默认选中他的所有子节点
          //只需要添加最底层的即可
